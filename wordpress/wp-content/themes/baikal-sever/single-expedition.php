...
<main>
  ...
  <section class="exp-booking section" id="booking">
    ...
    <form class="form" id="requestForm" action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post"><input type="hidden" name="action" value="baikal_submit_request"><input type="hidden" name="nonce" value="<?php echo esc_attr( wp_create_nonce( 'baikal_request' ) ); ?>"><input type="hidden" name="source" value="<?php echo esc_attr( get_the_title() ); ?>"><input class="form-honeypot" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true"><label>Ваше имя<input type="text" name="name" placeholder="Как к вам обращаться?"></label><label>Телефон<input type="tel" name="phone" placeholder="+7 (___) ___-__-__" required></label><label>Желаемая дата<input type="date" name="date"></label><button class="button button-accent submit" type="submit">Отправить заявку <span>↗</span></button><small>Нажимая кнопку, вы соглашаетесь с политикой конфиденциальности.</small></form>
  </section>
</main>
